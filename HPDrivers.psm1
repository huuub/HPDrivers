function Get-HPDrivers {
    ...
    if (($Manufacturer -match "HP") -or ($Manufacturer -match "Hewlett-Packard")) {
        ...
        if (!$ShowSoftware) { $AvailableDrivers = Get-SoftpaqList -OsVer 22H2 -Category BIOS, Driver }
        if ($ShowSoftware) { $AvailableDrivers = Get-SoftpaqList -OsVer 22H2 -Category BIOS, Driver, Diagnostic, Dock, Software, Utility }
